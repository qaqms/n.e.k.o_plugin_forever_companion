...
// + 我们的一月（月报，可翻历史月份）+ 相处徽章（收藏墙）+ 生日设置卡（同页最底部）。
// hosted-tsx 约束：唯一 export 排在任何 JSX 闭合标签之前；辅助组件放文件尾部靠函数声明提升
import { Card, EmptyState, StatusBadge, useEffect, useRef, useState } from "@neko/plugin-ui"
import type { BirthdayView, HeatDay, Heatmap, MonthReport, StatsBadge, StatsSummary, TFunc } from "./types"
...
export function MomentPane(props: {
  ...
  // 生日设置卡（1.3.1）：dashboard 的 birthday 视图 + 保存回调（复用 update_settings）
  birthday?: BirthdayView
  canSaveBirthday?: boolean
  onSetBirthday?: (date: string, keepDiary: boolean) => Promise<boolean>
}) {
  const { t, lanlan, summary, badges, heatmap, month, monthAvailable, monthLoading, onPickMonth, onPickYear,
    birthday, canSaveBirthday, onSetBirthday } = props
  ...
  return (
    <div className="tm-pane">
      ...
      {/* ---- 生日设置卡：时光页最底部（1.3.1 改版：展示型倒数退役，纯设置项压页尾） ---- */}
      <BirthdaySettingsCard
        t={t}
        birthday={birthday}
        canSave={canSaveBirthday !== false}
        onSave={onSetBirthday || (async () => false)}
      />
    </div>
  )
}
...
// 生日设置卡（1.3.1 改版·二次）：纯设置项——文本框式日期按钮展开**自绘月历**，
// 选日→「确认」直接落盘。原生 date input 整套退役：其内建弹层是浏览器 UI，
// 「今天」按钮既不能改文案也不能改行为，观感也和面板玻璃拟态割裂。
// 月历是**内嵌展开**（长在卡片正常流里，卡片随之变高），不是绝对定位浮层——
// Card 的玻璃层（overflow/backdrop-filter）会把浮层裁没（实机踩坑 2026-09-14）。
// 交互口径：确认=保存（旧「保存」按钮退役，走 update_settings 定向字段，不新建动作）；
// 纪念手记开关在日期已设时即时保存（TmSwitch 乐观回滚契约），未设日期时只存本地草稿、
// 随确认一并提交；清除只在已设日期时出现（空串 = 清除并休眠）；取消关面板并
// 丢弃草稿。未来日期不可选（生日不存在”还没过到的那一年”）。真校验在后端（发码前端
// 翻译，i18n 契约）。能力开关在「功能」页，这里只管“记不记得、留不留念”。
function BirthdaySettingsCard(props: {
  t: TFunc
  birthday?: BirthdayView
  canSave: boolean
  ...
  async function onToggleKeep(v: boolean) {
    setDraftKeep(v)
    // 已设日期 → 即时保存（失败 resolve false，TmSwitch 乐观回滚契约自动翻回）；
    // 未设日期 → 草稿态，随确认一并提交
    if (isSet && saved) {
      setBusy(true)
